    ...
    return (
      <main className="min-h-screen bg-slate-950 text-white flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center bg-slate-900 border border-white/10 rounded-2xl p-8">
          ...
          <p className="text-slate-300 mb-6">Форма работает! На следующем шаге подключим сервер, и аккаунт будет создаваться по-настоящему.</p>
    ...
    <main className="min-h-screen bg-slate-950 text-white py-12 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400">Уже есть аккаунт? <Link href="/login" className="text-violet-400 hover:text-violet-300 underline">Войти</Link></p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-slate-900 border border-white/10 rounded-2xl p-6 sm:p-8 space-y-6">
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Как к вам обращаться?" className="w-full rounded-xl bg-slate-800 border border-white/10 px-4 py-3 outline-none focus:border-violet-500" />
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Минимум 6 символов" className="w-full rounded-xl bg-slate-800 border border-white/10 px-4 py-3 outline-none focus:border-violet-500" />
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <div>
              ...
              <input type="date" value={birthDate} onChange={(e) => setBirthDate(e.target.value)} className="w-full rounded-xl bg-slate-800 border border-white/10 px-4 py-3 outline-none focus:border-violet-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">Время (необязательно)</label>
              <input type="time" value={birthTime} onChange={(e) => setBirthTime(e.target.value)} className="w-full rounded-xl bg-slate-800 border border-white/10 px-4 py-3 outline-none focus:border-violet-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">Город (необязательно)</label>
              <input type="text" value={birthCity} onChange={(e) => setBirthCity(e.target.value)} placeholder="Москва" className="w-full rounded-xl bg-slate-800 border border-white/10 px-4 py-3 outline-none focus:border-violet-500" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {HOROSCOPE_TYPES.map((t) => (
                <button key={t.slug} type="button" onClick={() => toggleType(t.slug)} className={`px-4 py-2 rounded-xl border text-sm font-medium transition ${selected.includes(t.slug) ? "bg-violet-600 border-violet-500 text-white" : "bg-slate-800 border-white/10 text-slate-300 hover:border-violet-500/50"}`}>
                  {t.icon} {t.name}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              {(["daily", "weekly"] as const).map((f) => (
                <button key={f} type="button" onClick={() => setFrequency(f)} className={`px-4 py-2 rounded-xl border text-sm font-medium transition ${frequency === f ? "bg-violet-600 border-violet-500 text-white" : "bg-slate-800 border-white/10 text-slate-300 hover:border-violet-500/50"}`}>
          ...
          <label className="flex items-start gap-3 text-sm text-slate-300 cursor-pointer">
            <input type="checkbox" checked={agree} onChange={(e) => setAgree(e.target.checked)} className="mt-1 accent-violet-600" />
            <span>Я согласен с <Link href="/privacy" className="text-violet-400 hover:text-violet-300 underline">политикой конфиденциальности</Link> и офертой</span>
          </label>

          {error && <div className="bg-red-500/10 border border-red-500/30 text-red-300 rounded-xl px-4 py-3 text-sm">{error}</div>}

          <button type="submit" className="w-full bg-violet-600 hover:bg-violet-500 rounded-xl px-6 py-4 font-semibold text-lg transition">Создать аккаунт — бесплатно</button>
